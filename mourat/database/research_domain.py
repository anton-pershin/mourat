...
def update_research_domain(
    ...
    values.append(id)
    conn.execute(
        f"UPDATE research_domains " f"SET {', '.join(fields)} " f"WHERE id = ?", values
    )
    conn.commit()
...
def create_research_direction(
    ...
    conn.execute(
        "INSERT INTO research_directions (id, name, description, domain_id) "
        "VALUES (?, ?, ?, ?)",
...
def get_research_direction(conn: sqlite3.Connection, id: str) -> dict | None:
    row = conn.execute(
        "SELECT * FROM research_directions WHERE id = ?", (id,)
    ).fetchone()
    return dict(row) if row else None
...
def update_research_direction(
    ...
    values.append(id)
    conn.execute(
        f"UPDATE research_directions SET {', '.join(fields)} WHERE id = ?", values
    )
    conn.commit()
...
def list_research_directions(
    conn: sqlite3.Connection, domain_id: str | None = None
) -> list[dict]:
    if domain_id:
        rows = conn.execute(
            "SELECT * FROM research_directions WHERE domain_id = ? ORDER BY id",
            (domain_id,),
        ).fetchall()
    else:
...
def create_research_object(
    ...
    conn.execute(
        "INSERT INTO research_objects (id, name, description, direction_id) "
        "VALUES (?, ?, ?, ?)",
...
def update_research_object(
    ...
    values.append(id)
    conn.execute(
        f"UPDATE research_objects SET {', '.join(fields)} WHERE id = ?", values
    )
    conn.commit()
...
def list_research_objects(
    conn: sqlite3.Connection, direction_id: str | None = None
) -> list[dict]:
    if direction_id:
        rows = conn.execute(
            "SELECT * FROM research_objects WHERE direction_id = ? ORDER BY id",
            (direction_id,),
        ).fetchall()
    else:
...
def create_research_question(
    conn: sqlite3.Connection, id: str, name: str, object_id: str, description: str = ""
) -> None:
    conn.execute(
        "INSERT INTO research_questions (id, name, description, object_id) "
        "VALUES (?, ?, ?, ?)",
...
def get_research_question(conn: sqlite3.Connection, id: str) -> dict | None:
    row = conn.execute(
        "SELECT * FROM research_questions WHERE id = ?", (id,)
    ).fetchone()
    return dict(row) if row else None
...
def update_research_question(
    ...
    values.append(id)
    conn.execute(
        f"UPDATE research_questions SET {', '.join(fields)} WHERE id = ?", values
    )
    conn.commit()
...
def list_research_questions(
    conn: sqlite3.Connection, object_id: str | None = None
) -> list[dict]:
    if object_id:
        rows = conn.execute(
            "SELECT * FROM research_questions WHERE object_id = ? ORDER BY id",
            (object_id,),
        ).fetchall()
    else:
...
def update_research_topic(
    ...
    values.append(id)
    conn.execute(
        f"UPDATE research_topics " f"SET {', '.join(fields)} " f"WHERE id = ?", values
    )
    conn.commit()
...
def add_topic_technical_challenge(
    conn: sqlite3.Connection, topic_id: str, challenge_id: str
) -> None:
    conn.execute(
        "INSERT INTO topic_technical_challenges (topic_id, challenge_id) "
        "VALUES (?, ?)",
...
def remove_topic_technical_challenge(
    conn: sqlite3.Connection, topic_id: str, challenge_id: str
) -> None:
    conn.execute(
        "DELETE FROM topic_technical_challenges "
        "WHERE topic_id = ? AND challenge_id = ?",
        (topic_id, challenge_id),
    )
    conn.commit()


def list_topic_technical_challenges(
...
def add_topic_research_question(
    conn: sqlite3.Connection, topic_id: str, question_id: str
) -> None:
    conn.execute(
...
def remove_topic_research_question(
    conn: sqlite3.Connection, topic_id: str, question_id: str
) -> None:
    conn.execute(
        "DELETE FROM topic_research_questions "
        "WHERE topic_id = ? AND question_id = ?",
        (topic_id, question_id),
    )
    conn.commit()


def list_topic_research_questions(
    conn: sqlite3.Connection, topic_id: str
) -> list[dict]:
    rows = conn.execute(
