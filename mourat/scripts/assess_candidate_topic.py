...
from mourat.assessors import CandidateTopicAssessor
...
def assess_candidate_topic(cfg: DictConfig) -> None:
    ...
    assessor: CandidateTopicAssessor = hydra.utils.instantiate(
        cfg.candidate_topic_assessor
    ...
    input_data = CandidateTopicInfo(
        ...
        business_product={
            p: BusinessProductInfo(**kwargs)
            for p, kwargs in cfg.business_products.items()
        },
    )

    # Step 1: assess relevance
    step_id = "1"
    assessor(input_data, step_id=step_id)
